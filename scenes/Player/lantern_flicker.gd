...
func _process(delta: float) -> void:
	...
	match phase:
		FlickerPhase.HOLD_LOW:
			energy = low_energy
			texture_scale = low_radius
		FlickerPhase.RISE:
			# Lantern snaps brighter quickly.
			energy = lerp(low_energy, peak_energy, _ease_out(t))
			texture_scale = lerp(low_radius, peak_radius, _ease_out(t))
		FlickerPhase.FALL:
			# Then decays right away and never lingers at the top.
			energy = lerp(peak_energy, low_energy, _ease_in(t))
			texture_scale = lerp(peak_radius, low_radius, _ease_in(t))
